...
public:
    Game()
        : level(1), score(0), gameOver(false), levelComplete(false), levelSeed(0), hasLevelSeed(false) {
        term.enableRawMode();
        term.hideCursor();
        generateLevel();
    }
    ...
    void generateLevel(bool reuseSeed = false) {
        ...
        // seed management:
        // - reuseSeed=true  => regenerate exactly same terrain
        // - reuseSeed=false => create a fresh terrain
        ...
        // 2. 生成横贯屏障层 (包含协作逻辑：无缺口，靠属性通过)
        int barrierStep = 4;
        for (int y = 5; y < HEIGHT - 3; y += barrierStep) {
            ...
            if (coopLayer) {
                ...
                for (int x = 1; x < WIDTH - 1; x++) {
                    if (x == doorX) {
                        ...
                        if (iceHelpsFire) {
                            map[y][x] = (R(10) < 7 ? WATER : WALL);
                        } else {
                            map[y][x] = (R(10) < 7 ? LAVA : WALL);
                        }
                    }
                }
                map[swY][swX] = SWITCH;
        ...
        for (int i = 0; i < numVerticalCols; i++) {
            ...
            int len = R(10) + 5;
            int typeRoll = R(3); // 仅 0:WALL, 1:WATER, 2:LAVA
    ...
    void processInput() {
        ...
        if (key == 'r' || key == 'R') { generateLevel(false); return; } // new map
    ...
    void showGameOver() {
        ...
        if (c == 'm' || c == 'M') {
            // replay same level with same terrain seed
            ...
            // full restart from level 1, fresh terrain
            score = 0;
            level = 1;
            fireBoy.isDead = false;
            iceGirl.isDead = false;
...
int main() {
    std::cout << "\033[2J\033[H" << std::flush;
    std::cout << "\n\n    Loading SSH Ice & Fire (v2.4: Clean Update)...\n\n";
    usleep(500000);
    Game game;
    game.run();
    return 0;
