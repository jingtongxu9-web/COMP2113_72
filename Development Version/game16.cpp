...
public:
    Game(int layers)
        : level(1), score(0), gameOver(false), levelComplete(false), levelSeed(0), hasLevelSeed(false), terrainLayers(layers) {
        term.enableRawMode();
        term.hideCursor();
        generateLevel();
    }
...
int showMainMenu() {
    while (true) {
        ...
        std::cout
            ...
            << "  1) Level 1 (3 terrain layers)\n"
            << "  2) Level 2 (5 terrain layers)\n"
            << "  3) Level 3 (7 terrain layers)\n\n"
            << "Enter 1, 2, or 3: ";
