...
private:
    ...
    static std::string cleanUsername(const std::string& rawName) {
        // Remove terminal control characters and common ANSI escape sequences.
        // This prevents damaged ranking_data.txt lines from creating invisible
        // players that appear as blank rows on the scoreboard.
        std::string cleaned;
    ...
    static bool parseRecordLine(const std::string& rawLine, std::string& username, int& score) {
        std::string line = trim(rawLine);
        if (line.empty()) return false;

        // Standard format written by this program: username<TAB>score.
        // Use the last tab so a damaged line with extra tabs still tries to use
        // the final field as the numeric score.
        size_t splitPos = line.find_last_of('\t');

        // Backward-compatible formats: username score or username,score.
    ...
    void addOrMergeRecord(const std::string& rawName, int score) {
        ...
        if (existingIdx != -1) {
            // If the same name appears multiple times because the file was
            // damaged or edited manually, keep the higher score.
...
public:
    ...
    bool loadFromFile(const std::string& filename) {
        ...
        // New format starts with a record count. If the first line is not a
        // plain integer, treat it as a legacy record line instead.
    ...
    bool saveToFile(const std::string& filename) const {
        // Save a clean version of the scoreboard. Invalid invisible names and
        // zero-score accidental menu inputs are not written back, so a corrupted
        // ranking_data.txt repairs itself after the next login/game session.
        ...
        std::sort(cleanRecords.begin(), cleanRecords.end(), [](const PlayerRecord& a, const PlayerRecord& b) {
...
private:
    ...
    // Difficulty 1 should be more beginner-friendly.
    // This also covers fixed levels 1-3, because their difficultyId is set to 1.
    bool isBeginnerDifficulty() const {
        return difficultyId == 1;
    }

    int getLayerGapHalfWidth() const {
        // Normal difficulties keep the old one-cell opening behavior.
        // Difficulty 1 uses a 5-cell opening: center gap + 2 cells on each side.
        return isBeginnerDifficulty() ? 2 : 0;
    }

    int getMinimumGapDistance() const {
        // Wider openings need more separation so two gaps do not overlap too often.
...
public:
    ...
    void generateLevel(bool reuseSeed = false) {
        ...
        // Difficulty 1, including fixed levels 1-3, should give players
        // wider and more forgiving layer openings. The rest of the procedural
            ...
            if (coopLayer) {
                ...
                for (int x = 1; x < WIDTH - 1; x++) {
                    ...
                    for (int gx : gaps) {
                        ...
                        // Keep the original small random widening only for higher difficulties.
    ...
    bool processInput() {
        ...
        // Random Challenge Mode only: F refreshes the current level with a new random map.
        // It keeps the current level number and previous completed-level score, but removes
        // any points earned inside the current unfinished map to avoid score farming.
    ...
    void run() {
        ...
        while (!gameOver) {
            ...
            if (levelComplete) {
                ...
                if (fixedLevelMode) {
                    // Fixed Level Mode now automatically advances to the next prepared level.
                    // The session only ends after fixed level 10 is completed, or after the
                    // player fails and chooses to quit/choose another fixed level.
                ...
                if (recordedRandomMode) {
                    // Recorded-random mode contains one selected map.
