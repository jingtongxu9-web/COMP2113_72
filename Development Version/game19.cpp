...
public:
    ...
    bool processInput() {
        ...
        if (dx != 0 || dy != 0) {
            ...
            if (tile == WALL || tile == DOOR_CLOSED) return false;
    ...
    void draw() {
        ...
        ss << "║  Tab: Switch  │  WASD/Arrows: Move  │  R: Retry Map │ Q: Quit      ║\n";
        ss << "╠══════════════════════════════════════════════════════════════════╣\n";
        ss << term.getResetStr();
    ...
    void showGameOver() {
        ...
        std::cout << "\n\n" << term.getColorStr(255, 50, 50)
                  ...
                  << "    ║                                       ║\n"
                  << "    ╚═══════════════════════════════════════╝\n" << term.getResetStr() << "\n"
                  << "    Press 'r' to replay same map | Press 'n' to restart from level 1 | Press 'q' to quit\n\n";
        ...
        if (c == 'r' || c == 'R') {
            ...
            // Start a completely new game from level 1 with a new random map.
            score = 0;
            level = 1;
            hasLevelSeed = false;
            fireBoy.isDead = false;
    ...
    void run() {
        ...
        while (!gameOver) {
            ...
            if (levelComplete) {
                ...
                // Lower refresh pressure on the terminal. Since the map is static,
                // we only redraw after input, which removes the constant flicker.
                usleep(30000);
            }
        }
        showGameOver();
