...
private:
    ...
    static bool parseRecordLine(const std::string& rawLine, std::string& username, int& score) {
        ...
        // Preferred current format: username<TAB>score.
        size_t tabPos = line.find('\t');
        if (tabPos != std::string::npos) {
            username = trim(line.substr(0, tabPos));
            std::string scoreStr = trim(line.substr(tabPos + 1));
            try {
        ...
        username = trim(line.substr(0, splitPos));
        std::string scoreStr = trim(line.substr(splitPos + 1));
        if (username.empty() || scoreStr.empty()) return false;
...
public:
    ...
    bool loadFromFile(const std::string& filename) {
        ...
        int expectedCount = 0;
        try {
            expectedCount = std::stoi(trim(firstLine));
        } catch (...) {
            expectedCount = -1;
        }
        ...
        // Normal file format starts with a count line. If the count is invalid,
        // still try to parse the first line as a legacy record instead of losing the scoreboard.
        if (expectedCount < 0) {
            std::string name;
            int sc = 0;
            if (parseRecordLine(firstLine, name, sc)) {
                expandIfNeeded();
                records[count].username = name;
                records[count].score = sc;
                count++;
            }
            expectedCount = std::numeric_limits<int>::max();
        }

        std::string line;
        int linesRead = 0;
        while (std::getline(in, line)) {
            if (linesRead >= expectedCount) break;
            linesRead++;

            std::string name;
            int sc = 0;
            if (!parseRecordLine(line, name, sc)) continue;

            int existingIdx = findUser(name);
            if (existingIdx != -1) {
                // If a damaged file has duplicate names, keep the higher score.
                records[existingIdx].score = std::max(records[existingIdx].score, sc);
                continue;
            }
