...
public:
    ...
    void draw() {
        ...
        else if (recordedRandomMode) levelText = "Saved " + std::to_string(recordedRecordId);
    ...
    void showLevelComplete() {
        ...
        else if (recordedRandomMode) completeText = "SAVED LEVEL " + std::to_string(recordedRecordId);
        ...
        while (true) {
            ...
            if (fixedLevelMode) {
                ...
                std::cout << "    Press 'r' to replay this recorded random level\n"
                          << "    Press 'o' to choose another recorded random level\n"
                          << "    Press 'q' to quit\n\n";
            } else {
...
void showMainMenu(const RankingSystem& ranking) {
    ...
    std::cout
        ...
        << "  3) Reload Recorded Random Level - replay a saved random-map ID\n\n"
...
int inputGameMode() {
    while (true) {
        std::cout << "Choose game mode - 1 Random, 2 Fixed, 3 Reload Random ID: ";
        char c;
        std::cin >> c;
        if (c == '1') return 1;
        if (c == '2') return 2;
...
int main() {
    ...
    while (true) {
        ...
        if (gameMode == 1) {
            ...
            {
                Game game(layers, reward, difficulty);
                game.run();
            ...
            while (chooseAnotherRecord) {
                ...
                if (records.empty()) {
                    std::cout << "\nNo recorded random levels found for this player.\n";
                    break;
                }
                ...
                if (!randomArchive.findRecordByIdForUser(username, recordId, selectedRecord)) {
                    std::cout << "Record not found. Returning to menu.\n";
                    break;
                }
