...
const int TERRAIN_LAYERS = 7; // 3 = easy, 5 = normal, 7 = hard
...
public:
    ...
    void generateLevel(bool reuseSeed = false) {
        ...
        // 2. 生成横贯屏障层（根据 TERRAIN_LAYERS 控制数量）
        std::vector<int> layerRows;
        if (TERRAIN_LAYERS == 3) {
            layerRows = {5, 13, 21};
        } else if (TERRAIN_LAYERS == 5) {
        ...
        for (int i = 0; i < numVerticalCols; i++) {
            ...
            int len = R(10) + 5;
            int typeRoll = R(3); // 0:WALL, 1:WATER, 2:LAVA
