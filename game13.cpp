...
public:
    ...
    void generateLevel() {
        ...
        for (int y = 5; y < HEIGHT - 3; y += barrierStep) {
            bool coopLayer = (level >= 3 && rand() % 100 < 50);

            if (coopLayer) {
                bool iceHelpsFire = (rand() % 2 == 0); 
                int doorX = iceHelpsFire ? (5 + rand() % 15) : (WIDTH - 20 + rand() % 15);
                int swX = iceHelpsFire ? (WIDTH - 20 + rand() % 15) : (5 + rand() % 15);
                int swY = y - 2;
                ...
                for (int x = 1; x < WIDTH - 1; x++) {
                    if (x == doorX) {
                        ...
                        if (iceHelpsFire) {
                            map[y][x] = (rand() % 10 < 7 ? WATER : WALL);
                        } else {
                            map[y][x] = (rand() % 10 < 7 ? LAVA : WALL);
                        }
                    }
                }
                map[swY][swX] = SWITCH;
                ...
                // 普通混合障碍层
                int gap1 = rand() % (WIDTH - 15) + 5;
                int gap2 = rand() % (WIDTH - 15) + 10;
                for (int x = 1; x < WIDTH - 1; x++) {
        ...
        for (int i = 0; i < numVerticalCols; i++) {
            int vx = rand() % (WIDTH - 10) + 5;
            int startY = rand() % (HEIGHT / 2);
            int len = rand() % 10 + 5;
            int typeRoll = rand() % 3; // 仅 0:WALL, 1:WATER, 2:LAVA
